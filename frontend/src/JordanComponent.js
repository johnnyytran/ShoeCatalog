...
import './JordanComponent.css';
import View from './view';

function JordanComponent({ backToHomepage }) {
  ...
  const fetchJordanShoes = async () => {
    try {
      const response = await fetch('http://localhost:8081/listJordan');
      if (!response.ok) {
        throw new Error('Failed to fetch Jordan shoes');
      }
      ...
      console.error('Error fetching Jordan shoes:', error);
    }
  };
  ...
  const handleAddToCart = async (shoeId) => {
    ...
    try {
      ...
      const shoe = jordanShoes.find(shoe => shoe.id === shoeId);
  ...
  return (
    <div className="jordan-container">
      ...
      <h1 className="Title">Jordan Selection</h1>
