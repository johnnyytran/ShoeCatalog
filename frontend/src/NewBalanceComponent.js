...
import './NewBalanceComponent.css';
import View from './view';

function NewBalanceComponent({ backToHomepage }) {
  ...
  useEffect(() => {
    fetchNewBalanceShoes();
    // Generate a random session ID when the component mounts
    const sessionId = generateSessionId();
    setSessionId(sessionId);
  }, []);

  const fetchNewBalanceShoes = async () => {
    try {
      const response = await fetch('http://localhost:8081/listNewBalance');
      if (!response.ok) {
        throw new Error('Failed to fetch NewBalance shoes');
      }
      ...
      setNewBalanceShoes(data);
    } catch (error) {
      console.error('Error fetching NewBalance shoes:', error);
    }
  };
  ...
  const handleAddToCart = async (shoeId) => {
    ...
    try {
      ...
      const shoe = newbalanceShoes.find(shoe => shoe.id === shoeId);
  ...
  return (
    <div className="newbalance-container">
      ...
      <h1 className="Title">NewBalance Selection</h1>
      <div className="shoe-list">
        {newbalanceShoes.map(shoe => (
