...
import './AdidasComponent.css';
import View from './view';

function AdidasComponent({ backToHomepage }) {
  ...
  const fetchAdidasShoes = async () => {
    try {
      const response = await fetch('http://localhost:8081/listAdidas');
      if (!response.ok) {
        throw new Error('Failed to fetch Adidas shoes');
      }
      ...
      console.error('Error fetching Adidas shoes:', error);
    }
  };
  ...
  const handleAddToCart = async (shoeId) => {
    ...
    try {
      ...
      const shoe = adidasShoes.find(shoe => shoe.id === shoeId);
  ...
  return (
    <div className="adidas-container">
      ...
      <h1 className="Title">Adidas Selection</h1>
...
export default AdidasComponent;
